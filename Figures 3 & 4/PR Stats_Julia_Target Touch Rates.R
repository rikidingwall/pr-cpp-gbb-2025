...
cleaned_d <- d[!is.na(d$Target_Touches_Rate) & d$Target_Touches_Rate != "" & d$Trial == "None", ]
cleaned_d <- cleaned_d[, c("Genotype", "Animal.ID", "Date.Time", "Trial", "Target_Touches_Rate")]
...
# Create Target Touch Rate and clean up data file 
d$Target_Touches_Rate <- d$Target_Touches / d$Schedule_Length
cleaned_d <- d[!is.na(d$Target_Touches_Rate) & d$Target_Touches_Rate != "" & d$Trial == "ATO", ]
cleaned_d <- cleaned_d[, c("Genotype", "Animal.ID", "Date.Time", "Drug", "Trial", "Target_Touches_Rate")]
...
# Fit the beta regression 
model <- glmmTMB(Target_Touches_Rate ~ Genotype * Drug * Date.Time + (1|Animal.ID), 
                 data = cleaned_d, 
                 family = beta_family)
cis <- confint(model, level = 0.95)
summary(model)
...
posthoc_model <- glmmTMB(Target_Touches_Rate ~ Genotype * Drug * Date.Time + (1|Animal.ID), 
                 data = cleaned_d, 
                 family = beta_family)
...
group_summary <- cleaned_d %>%
  ...
  summarise(Mean = mean(Target_Touches_Rate), SEM = sd(Target_Touches_Rate)/sqrt(n())) %>%
  ungroup()
...
ggplot(group_summary, aes(x = Date.Time, y = Mean, group = Genotype_Drug, color = Genotype_Drug)) +
  ...
  labs(title = "Line Graph of Mean Target Touches Rate by Date", x = "Date", y = "Mean Target Touches Rate") +
  theme_minimal() +
...
# Create Target Touch Rate and clean up data file
d$Target_Touches_Rate <- d$Target_Touches / d$Schedule_Length
cleaned_d <- d[!is.na(d$Target_Touches_Rate) & d$Target_Touches_Rate != "" & d$Trial == "MPH", ]
cleaned_d <- cleaned_d[, c("Genotype", "Animal.ID", "Date.Time", "Drug", "Trial", "Target_Touches_Rate")]
...
# repeat for FR2, read in data file
d = read.csv ("/Volumes/LaCie/Florey PhD/Original Papers/PR-ATO & MPH/Data/Spreadsheets/Touchscreens/Julia 2018/Julia_FR and ERC.csv")

# Create Target Touch Rate, clean up data file 
d$Target_Touches_Rate <- d$Target_Touches / d$Schedule_Length
...
# repeat for FR3, load in FR data file
d = read.csv ("/Volumes/LaCie/Florey PhD/Original Papers/PR-ATO & MPH/Data/Spreadsheets/Touchscreens/Julia 2018/Julia_FR and ERC.csv")
...
# Perform posthoc test for interaction effect
cleaned_d$Date.Time <- as.factor(cleaned_d$Date.Time)
posthoc_model <- glmmTMB(Target_Touches_Rate ~ Genotype * Date.Time + (1|Animal.ID), 
                 data = cleaned_d, 
                 family = beta_family)
...
# repeat for FR5 during PR + ATO, read in data file
d = read.csv ("/Volumes/LaCie/Florey PhD/Original Papers/PR-ATO & MPH/Data/Spreadsheets/Touchscreens/Julia 2018/Julia_FR and ERC.csv")
d <- d %>%
...
cleaned_d <- d[!is.na(d$Target_Touches_Rate) & d$Target_Touches_Rate != "" & d$Trial == "None", ]
cleaned_d <- cleaned_d[, c("Genotype", "Animal.ID", "Date.Time", "Trial", "Target_Touches_Rate")]
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/julia-pr-target-touch-rate.xlsx")

# Export FR data file
d = read.csv ("/Volumes/LaCie/Florey PhD/Original Papers/PR-ATO & MPH/Data/Spreadsheets/Touchscreens/Julia 2018/Julia_FR and ERC.csv")
d$Target_Touches_Rate <- d$Target_Touches / d$Schedule_Length
cleaned_d <- d[!is.na(d$Target_Touches_Rate), ]
cleaned_d <- cleaned_d[, c("Genotype", "Animal.ID", "Date.Time", "Target_Touches_Rate")]
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/julia-fr-target-touch-rate.xlsx")
