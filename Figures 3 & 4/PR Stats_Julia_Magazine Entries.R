...
# Clean up data file
cleaned_d <- d[!is.na(d$Magazine_Entries) & d$Magazine_Entries != "" & d$Trial == "None", ]
...
# Clean up data file
cleaned_d <- d[!is.na(d$Magazine_Entries) & d$Magazine_Entries != "" & d$Trial == "ATO", ]
...
group_summary <- cleaned_d %>%
  ...
  summarise(Mean = mean(Magazine_Entries), SEM = sd(Magazine_Entries)/sqrt(n())) %>%
  ungroup()
...
ggplot(group_summary, aes(x = Date.Time, y = Mean, group = Genotype_Drug, color = Genotype_Drug)) +
  ...
  labs(title = "Line Graph of Mean Magazine Entries by Date", x = "Date", y = "Mean Magazine Entries") +
  theme_minimal() +
...
# Clean up data file
cleaned_d <- d[!is.na(d$Magazine_Entries) & d$Magazine_Entries != "" & d$Trial == "MPH", ]
