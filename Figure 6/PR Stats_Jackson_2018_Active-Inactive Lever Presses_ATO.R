...
d = read.csv ("/Volumes/LaCie/Florey PhD/Original Papers/PR-ATO & MPH/Data/Spreadsheets/Operant/Jackson_2018_PR.csv")
...
# Export data file
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/jackson_2018_fr1-sl.xlsx")
...
# Export data file
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/jackson_2018_fr1-dl.xlsx")
...
# Export data file
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/jackson_2018_fr3.xlsx")
...
# Restrict analysis to trials prior to devaluation
cleaned_d <- cleaned_d[cleaned_d$Session_Number <= 4, ]

# Restrict analysis to trials between devaluation and PR 
cleaned_d <- cleaned_d[cleaned_d$Session_Number >= 7 & cleaned_d$Session_Number <= 12, ]

# Restrict analysis to baselining trials during PR 
cleaned_d <- cleaned_d[cleaned_d$Session_Number < 12, ]
...
# Export data file
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/jackson_2018_fr5.xlsx")
...
# Read in CSV for FR5 DEVAL
d = read.csv ("/Volumes/LaCie/Florey PhD/Original Papers/PR-ATO & MPH/Data/Spreadsheets/Operant/Jackson_2018_PR.csv")

# Only keep rows where Stage is "FR5 DEVAL"
cleaned_d <- d[d$Stage == "FR5 DEVAL", ]
...
# Calculate the mean and SEM for each combination of Genotype and Date.Time
active_grouped_data <- cleaned_d %>%
  group_by(Genotype, Session_Number) %>%
  summarise(
    Active_Mean = mean(Active_Lever_Presses),
...
active.plot <- ggplot(active_grouped_data, aes(x = Genotype, y = Active_Mean, fill = Genotype, group = Session_Number)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
  geom_errorbar(aes(ymin = Active_Mean - Active_SEM, ymax = Active_Mean + Active_SEM, group = Session_Number), 
                position = position_dodge(width = 0.8), width = 0.25) +
  scale_fill_manual(values = c("WT" = "black", "KI" = "blue")) +
  facet_wrap(~Session_Number) + 
  labs(x = "Genotype", y = "Active Lever Presses (Mean ± SEM)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

inactive.plot <- ggplot(inactive_grouped_data, aes(x = Genotype, y = Inactive_Mean, fill = Genotype, group = Session_Number)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
  geom_errorbar(aes(ymin = Inactive_Mean - Inactive_SEM, ymax = Inactive_Mean + Inactive_SEM, group = Session_Number), 
                position = position_dodge(width = 0.8), width = 0.25) +
  scale_fill_manual(values = c("WT" = "black", "KI" = "blue")) +
  facet_wrap(~Session_Number) + 
  labs(x = "Genotype", y = "Inactive Lever Presses (Mean ± SEM)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Export data file
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/jackson_2018_fr5-deval.xlsx")
...
# Export data file
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/jackson_2018_pr.xlsx")
...
# Run post-hoc analysis for dose by genotype interaction
cleaned_d$Dose <- as.factor(cleaned_d$Dose)
posthoc_model <- glmmTMB(Active_Lever_Presses ~ Genotype * Dose + (1|Animal.ID), 
                         data = cleaned_d, 
                         family = poisson)
emm_int <- emmeans(posthoc_model, ~ Genotype | Dose)
posthoc_results <- pairs(emm_int)
posthoc_cis <- confint(posthoc_model, level = 0.95)
summary(posthoc_results)
print(posthoc_cis)

# Fit GLMM with poisson regression for dose effect
cleaned_d$Dose <- as.factor(cleaned_d$Dose)
cleaned_d$Dose <- factor(cleaned_d$Dose, levels= c("0", "1", "3"))
cleaned_d$Drug_Dose <- interaction(cleaned_d$Drug, cleaned_d$Dose)
posthoc_model <- glmmTMB(Active_Lever_Presses ~ Genotype * Drug_Dose + (1|Animal.ID), 
                 data = cleaned_d, 
                 family = poisson)
emm_int <- emmeans(posthoc_model, ~ Drug_Dose)
posthoc_results <- pairs(emm_int)
posthoc_cis <- confint(posthoc_model, level = 0.95)
summary(posthoc_results)
print(posthoc_cis)
...
# Export data file
write_xlsx(cleaned_d, "/Users/rikidingwall/Downloads/jackson_2018_pr-ato.xlsx")
